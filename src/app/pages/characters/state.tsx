...
type CharactersListEntry = Pick<Character, 'id' | 'name' | 'xp' | 'classId' | 'retired'>;
...
export function useCharactersList() {
  ...
  const [performDeleteMutation, { status: deleteStatus, error: deleteError }] = useMutation(
    async (characterId: string) => {
      return firebase.firestore().collection('characters').doc(characterId).delete();
    },
    {
      onMutate: (characterId) => {
        ...
        const previousCharacters = queryCache.getQueryData<CharactersList>(charactersListQueryKey);
        const previousCharacter = queryCache.getQueryData<Character>(characterQueryKey);
  ...
  const [performAddMutation, { status: addStatus, error: addError }] = useMutation(
  ...
  const dispatchCharactersListAction = React.useCallback(
    (action: CharactersListAction) => {
      switch (action.type) {
        case 'add':
          performAddMutation(makeNewCharacter());
          break;
        case 'delete':
