...
const getNextState = produce<(draftState: Character, action: CharacterAction) => void>(
  (draftState, action) => {
    switch (action.type) {
...
export function useCharacter(characterId: string) {
  ...
  const { status, data: character, error } = useQuery(queryKey, async () => {
    const snapshot = await firebase.firestore().collection('characters').doc(characterId).get();
