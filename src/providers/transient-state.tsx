...
const transientStateReducer = produce(
  (draftState: TransientState, action: { type: TransientActionType; payload?: any }) => {
    switch (action.type) {
...
const transientStateContext = React.createContext<
  [TransientState, React.Dispatch<{ type: TransientActionType; payload?: any }>] | undefined
>(undefined);
const TransientStateContextProvider = transientStateContext.Provider;
export function TransientStateProvider({ children }) {
  const [state, dispatch] = React.useReducer(transientStateReducer, initialState);
