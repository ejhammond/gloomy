...
function aspectToColor(aspect: AttackModifierCardType['primaryAspect']): keyof typeof colors {
  switch (aspect.type) {
    case 'damage':
      if (aspect.value < 0) {
        return 'red';
...
function aspectToLayer(
  ...
  throw new Error('Aspect ' + aspect.type + ' does not have a corresponding layer');
}
...
export function FullAttackModifierCard({ card }: { card: AttackModifierCardType }) {
  ...
  return (
    <AspectRatio aspectRatio={296 / 437}>
      ...
      >
        ...
        />
        {['damage', 'miss', 'critical', 'blessing', 'curse'].includes(primaryAspect.type) ? (
